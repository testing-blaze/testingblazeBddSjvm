...
public final class Waits {
    ...
    public Waits(BrowsingDeviceBucket device, FrameManager iframeAnalyzer, JavaScript js,ReportController reportController,SwitchTo switchTo) {
    ...
    public Boolean disappearForProcessingONLY(By locator, long customWaitTime) {
        if (waitToInvisible == null) {
            waitToInvisible = new WebDriverWait(device.getDriver(), customWaitTime).pollingEvery(Duration.ofSeconds(STANDARD_POLLING_TIME));
        }
    ...
    private <V> V waitFor(ExpectedCondition<V> expectedCondition, long waitTime) {
        ...
        V value;
        if (wait == null) {
            wait = new WebDriverWait(device.getDriver(), STANDARD_WAIT_TIME).pollingEvery(Duration.ofSeconds(STANDARD_POLLING_TIME)).ignoring(NoSuchElementException.class, NullPointerException.class);
        }
        reportController.write(LogLevel.INFO, "Waiting for " + expectedCondition.toString());
        ...
        try {
            ...
            reportController.write(LogLevel.ERROR, "Wait for " + expectedCondition.toString() + " failed.");
            throw e;
        } finally {
            long postTime = System.currentTimeMillis();
            reportController.write(LogLevel.INFO, String.format("%s after %.1f  seconds.", result, ((postTime - preTime) / 1_000.0)));
        }
        return value;
    }
    ...
    private <V> V waitFor(By locator, ExpectedCondition<V> expectedCondition, long waitTime) {
        ...
        switchTo.acceptAlert();
        js.scrollElementToPageDetailCenter(locator);
        return value;
    }
    ...
    private <V> V waitFor(WebElement element, ExpectedCondition<V> expectedCondition, long waitTime) {
        switchTo.acceptAlert();
        js.scrollElementToPageDetailCenter(element);
