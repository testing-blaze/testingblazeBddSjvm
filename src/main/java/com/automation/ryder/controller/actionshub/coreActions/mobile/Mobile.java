...
public final class Mobile {
    ...
    public Mobile(BrowsingDeviceBucket device, Android android,IOS ios) {
        this.device=device;
    ...
    private AppiumDriver<WebElement> driver() {
        if (!"android".equalsIgnoreCase(EnvironmentFactory.getBrowsingDevice())
                && !"ios".equalsIgnoreCase(EnvironmentFactory.getBrowsingDevice())) {
            throw new RyderFrameworkException("In order to use 'Mobile' library, System parameter 'device' must be set to 'android' or 'ios'.\n" +
                    "Parameter 'device' was found to be '" + EnvironmentFactory.getBrowsingDevice() + "'");
        }
        return (AppiumDriver<WebElement>) device.getDriver();
    }

    private TouchAction<?> touch() {
        if ("android".equalsIgnoreCase(EnvironmentFactory.getBrowsingDevice())) {
            return new TouchAction<AndroidTouchAction>(driver());
        } else if ("ios".equalsIgnoreCase(EnvironmentFactory.getBrowsingDevice())) {
